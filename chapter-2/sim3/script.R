...
s1 = sim$get_counts()$counts[2]/sim$get_counts()$counts[1]
s2 = abs(sim$get_counts()$counts[2]-sim$get_counts()$counts[1])
sum_stat_obs = c(s1, s2)

p <- plot_tissue(sim, num_of_bins = 50)
...
create_model <- function(gr, tf){
    model=function(x){
        if (all(x > 0)){
            ...
            st1 = tmp$get_counts()$counts[2]/tmp$get_counts()$counts[1]
            st2 = abs(tmp$get_counts()$counts[2]-tmp$get_counts()$counts[1])
            s=c(st1, st2)
        } else {
            s=c(0,0)
        }
